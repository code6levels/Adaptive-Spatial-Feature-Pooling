...
class VOC12ClsDatasetMSF(VOC12ClsDataset):
    ...
    def __getitem__(self, idx):
        ...
        print(f'lenth = {len(ms_img_list)}')

        msf_img_list = []
        for i in range(len(ms_img_list)):
            msf_img_list.append(ms_img_list[i])
            msf_img_list.append(torch.flip(ms_img_list[i], -1).copy())

        return name, msf_img_list, label

class ExtractTripletLabelInRadius():
    ...
    def __init__(self, cropsize, radius=5):
        ...
        self.crop_height = cropsize - 2 * self.radius_floor
        self.crop_width = cropsize - 2 * self.radius_floor
        return

    def __call__(self, label):

        labels_from = label[self.radius_floor:-self.radius_floor, self.radius_floor:-self.radius_floor]
        ...
        for dy, dx in self.search_dist:
            labels_to = label[self.radius_floor+dy:self.radius_floor+dy+self.crop_height,\
        ...
        pos_affinity_label = np.equal(bc_labels_from, concat_labels_to).astype(np.float32)

        neg_affinity_label = np.logical_and(np.logical_not(pos_affinity_label), concat_valid_pair).astype(np.float32)

        return torch.from_numpy(pos_affinity_label), torch.from_numpy(neg_affinity_label)
...
class ExtractAffinityLabelInRadius_mask():
    ...
    def __call__(self, label, bg_pre=None, fg_pre=None, neg_pre=None):
        ...
        bg_pos_affinity_label = np.logical_and(pos_affinity_label, np.equal(bc_labels_from, 0))
        if bg_pre is not None:
            bg_pos_affinity_label = np.logical_and(bg_pos_affinity_label, np.logical_not(bg_pre))

        fg_pos_affinity_label = np.logical_and(np.logical_and(pos_affinity_label, np.not_equal(bc_labels_from, 0)), concat_valid_pair)
        if fg_pre is not None:
            fg_pos_affinity_label = np.logical_and(fg_pos_affinity_label, np.logical_not(fg_pre))

        neg_affinity_label = np.logical_and(np.logical_not(pos_affinity_label), concat_valid_pair)
        if neg_pre is not None:
            neg_affinity_label = np.logical_and(neg_affinity_label, np.logical_not(neg_pre))    

        return torch.from_numpy(bg_pos_affinity_label.astype(np.float32)),\
               torch.from_numpy(fg_pos_affinity_label.astype(np.float32)),\
               torch.from_numpy(neg_affinity_label.astype(np.float32))
