...
def do_python_eval(predict_folder,args,epoch):
    f = open(f'{args.session_name}/results.txt', 'a')
    gt_folder = '/input/VOC2012/VOCdevkit/VOC2012/SegmentationClass'
    # gt_folder = '/input/VOC2012/VOCdevkit/VOC2012/SegmentationClassAug/'

    num_cls = 21
    df = pd.read_csv('/input/VOC2012/VOCdevkit/VOC2012/ImageSets/Segmentation/val.txt', names=['filename'])
    # df = pd.read_csv('/root/zh/deeplabv3plus-pytorch-master/data/train_val.txt', names=['filename'])
    ...
    for i in range(num_cls):
        if i%2 != 1:
            ...
            print('%11s:%7.3f%%' % (categories[i], IoU[i] * 100), end='\t',file=f)
        else:
            print('%11s:%7.3f%%'%(categories[i],IoU[i]*100))
            print('%11s:%7.3f%%' % (categories[i], IoU[i] * 100),file=f)
    ...
    print('\n======================================================',file=f)
    print('%11s:%7.3f%%' % ('mIoU', miou * 100),file=f)
    print('%11s:%7.3f' % ('T/TP', t_tp),file=f)
    print('%11s:%7.3f' % ('P/TP', p_tp),file=f)
    print('%11s:%7.3f' % ('FP/ALL', fp_all),file=f)
    print('%11s:%7.3f' % ('FN/ALL', fn_all),file=f)
    print('%11s:%7.3f' % ('miou_foreground', miou_foreground),file=f)
    print('\n', file=f)
...
if __name__ == '__main__':
    ...
    name_list = df['filename'].values
    loglist = do_python_eval(args.predict_dir, args.gt_dir, 21)
